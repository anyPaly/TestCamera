...
class PhotoUtils {
    ...
    companion object {
        ...
        private val authorities = "com.example.testcamera.fileprovider"
        ...
        private fun crop(manager: FragmentManager, uri: Uri, aspectX: Int, aspectY: Int, outputX: Int, outputY: Int, cropCallBack: (Uri?, Boolean, String) -> Unit) = getPhotoFragment(manager).crop(uri, aspectX, aspectY, outputX, outputY, cropCallBack)
        ...
        fun crop(uri: Uri, aspectX: Int, aspectY: Int, outputX: Int, outputY: Int, callBack: (Uri?, Boolean, String) -> Unit) {
            ...
            val uri1 = if (uri.scheme.equals(ContentResolver.SCHEME_FILE)) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    FileProvider.getUriForFile(requireContext(), authorities, uriToFileQ(requireContext(), uri)!!)
                } else {
                    return
                }
            }
            ...
            cursor?.let {
                ...
                outUri = Uri.fromFile(File(requireContext().externalCacheDir!!.absolutePath, "${Random.nextInt(0, 9999)}$displayName"))
        ...
        override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                when (requestCode) {
                    ...
                    REQUEST_CODE_CAMERA -> {
                        val uri = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            FileProvider.getUriForFile(requireContext(), authorities, File(cameraPath))
                        } else {
